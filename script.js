 ...
 * 
 * Architecture:
 * - Central state management with immutability
...
function addProject(projectData = null) {
  ...
  card.addEventListener('dragstart', (e) => {
    e.dataTransfer.setData('text/plain', Array.from(DOM.projectsList.children).indexOf(card));
  });
  
  card.addEventListener('dragover', (e) => {
    e.preventDefault();
    const afterElement = getDragAfterElement(DOM.projectsList, e.clientY);
    if (afterElement && afterElement !== card) {
      DOM.projectsList.insertBefore(card, afterElement);
...
function downloadAllFiles() {
  ...
  if (typeof JSZip !== 'undefined') {
    ...
    // Add README
    const readme = `# ${AppState.data.fullName || 'My'} Portfolio
...
## Customization
Edit the HTML, CSS, or JavaScript files to customize further.
...
Built with ❤️ using Portfolio Builder`;
    ...
    showToast('📄 Files downloaded individually');
  }
}
...
function setupEventListeners() {
  ...
  document.addEventListener('keydown', (e) => {
    // Escape closes modals and preview
    if (e.key === 'Escape') {
...
function renderAll() {
  ...
  DOM.accentColor.value = '#d46a4a';
  DOM.accentColorHex.value = '#d46a4a';
  DOM.borderRadius.value = 12;
  DOM.radiusValue.textContent = '12px';
