/**
 * Portfolio Builder — Complete Application
 * 
 * Features:
 * - 6-step setup wizard
 * - Live preview with device switching
 * - Project management (add, remove, duplicate)
 * - Skill tags
 * - Social links
...
function navigateTo(step) {
  ...
  document.querySelectorAll('.nav-item').forEach(el => {
    el.classList.toggle('active', el.dataset.step === step);
    el.setAttribute('aria-selected', el.dataset.step === step);
  });
...
function collectData() {
  ...
  AppState.data.aboutImage = sanitizeString(DOM.aboutImage.value.trim());
  ...
  document.querySelectorAll('.project-card').forEach(card => {
    AppState.data.projects.push({
      ...
      image: sanitizeString(card.querySelector('.project-image-input')?.value || ''),
      github: sanitizeString(card.querySelector('.project-github-input')?.value || ''),
      live: sanitizeString(card.querySelector('.project-live-input')?.value || ''),
  ...
  document.querySelectorAll('.social-input-row').forEach(row => {
    ...
    const url = sanitizeString(row.querySelector('.social-url')?.value || '');
    if (platform && url) {
  ...
  // Design
  AppState.data.design.theme = document.querySelector('.theme-option.active')?.dataset.theme || 'light';
  ...
  AppState.data.design.borderRadius = parseInt(DOM.borderRadius.value);
...
function addProject(projectData = null) {
  ...
  card.querySelectorAll('input').forEach(input => {
    input.addEventListener('input', () => { renderPreview(); updateStatus(); });
    input.addEventListener('change', () => { renderPreview(); updateStatus(); });
  });
...
function duplicateProject(card) {
  const p = {
      ...
      <div class="project-card ${featuredClass}">
        ...
        <div class="project-info">
          ...
          <div class="project-links">
            ...
            ${p.live ? `<a href="${escapeHTML(p.live)}" target="_blank">Live Demo</a>` : ''}
          </div>
        </div>
      </div>
    `;
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .site-header {
      ...
      background: rgba(var(--bg-rgb, 255,255,255), 0.85); backdrop-filter: blur(16px);
    ...
    .project-card { background: var(--card); border-radius: var(--radius); overflow: hidden; ${cardStyle} transition: all 0.3s; }
...
function generatePortfolio() {
  ...
  AppState.generatedFiles = { html, css, js };
...
function downloadAllFiles() {
  ...
  if (typeof JSZip !== 'undefined') {
    ...
    downloadFile(files.html, 'index.html');
    setTimeout(() => downloadFile(files.css, 'style.css'), 300);
    setTimeout(() => downloadFile(files.js, 'script.js'), 600);
  }
}
...
function downloadSingle(type) {
  ...
  const map = {
    html: { content: files.html, filename: 'index.html', type: 'text/html' },
    css: { content: files.css, filename: 'style.css', type: 'text/css' },
    js: { content: files.js, filename: 'script.js', type: 'application/javascript' }
  };
...
function setupEventListeners() {
  ...
  // Export
  DOM.exportBtn.addEventListener('click', downloadAllFiles);
  ...
  DOM.showCodeBtn.addEventListener('click', () => {
    ...
    DOM.showCodeBtn.textContent = isVisible ? 'View Generated Code' : 'Hide Generated Code';
  });
  ...
  DOM.outputTabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      if (files) {
        ...
        DOM.outputContent.textContent = map[type] || '';
      }
    });
  });
